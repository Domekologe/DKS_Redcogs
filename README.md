...
| birthday | Release 0.1.0 | Birthday announcements + optional birthday role (self-healing). Opt-in per guild, DE/EN, dashboard panel. | `birthday set/remove/list`, `birthdayset enable/channel/role/hour/message/language` | Domekologe |
| statchannels | Release 0.1.0 | Live counter / stat voice channels (`{members}`, `{humans}`, `{bots}`, `{online}`, `{boosts}`, `{roles}`, `{channels}`). Opt-in, DE/EN, dashboard. | `statchannels enable/add/remove/list/language` | Domekologe |
| scheduledmsg | Release 0.1.0 | Scheduled & recurring messages (`every`/`daily`/`weekly`/`once`). Opt-in per guild, DE/EN, dashboard. | `schedule add/list/remove/enable/language` | Domekologe |
| eventlog | Release 0.1.0 | Server event logging (joins/leaves, msg edit/delete, role/nick/voice) to a channel with per-type toggles. Opt-in, DE/EN, dashboard. | `eventlog enable/channel/event/status/language` | Domekologe |
| leveling | Release 0.1.0 | XP / leveling with rank cards (image + embed fallback), leaderboard and level-role rewards. Opt-in per guild, DE/EN, dashboard. | `rank`, `leaderboard`, `xpset enable/cooldown/xprange/announce/levelrole/noxp/language` | Domekologe |
