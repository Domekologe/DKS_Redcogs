...
This is the cog guide for the ``DKS-Dashboard`` cog. This guide contains the collection of commands which you can use in the cog.
Through this guide, ``[p]`` will always represent your prefix. Replace ``[p]`` with your own prefix when you use these commands in Discord.

.. note::

    Ensure that you are up to date by running ``[p]cog update dks_dashboard``.
...
Now, we can install DKS-Dashboard.

.. code-block:: ini

    [p]cog install AAA3A-cogs dks_dashboard

Once it's installed, it is not loaded by default. Load it by running the following command:

.. code-block:: ini

    [p]load dks_dashboard
