...
class DashboardRPC:
    ...
    async def check_version(self) -> typing.Dict[str, int]:
        cog = self.bot.get_cog("DKS-Dashboard") or self.bot.get_cog("Dashboard")
        return {"version": cog.rpc.version}
    ...
    async def get_data(self) -> typing.Dict[str, typing.Any]:
        ...
        # Expose third-party pages directly in the sidebar for tighter integration.
        third_party_pages = []
        disabled_third_parties = set(data.get("disabled_third_parties", []))
        for third_party_name, pages in self.third_parties_handler.third_parties.items():
            if third_party_name in disabled_third_parties:
                continue
            for page_name in pages:
                third_party_pages.append(
                    {
                        "name": f"third-party-{third_party_name}-{page_name}",
                        "display_name": f"{third_party_name} - {page_name}",
                        "icon": "ni ni-app text-info",
                        "route": "third_parties_blueprint.third_party",
                        "session": True,
                        "owner": False,
                        "locked": False,
                        "hidden": False,
        ...
        data["ui"]["integrated_third_parties"] = third_party_pages
