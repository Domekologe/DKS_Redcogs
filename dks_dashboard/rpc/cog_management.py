...
class FakeContext:
    ...
    def get_notifications(self) -> typing.List[typing.Dict[str, str]]:
        ...
        for line in content.split("\n"):
            ...
            elif line.lstrip().startswith(("-", "**", "`")) and notifications:
...
class DashboardRPC_CogManagement:
    ...
    async def _safe_downloader_call(
        self,
        action: typing.Callable[..., typing.Awaitable[typing.Any]],
        *args: typing.Any,
        **kwargs: typing.Any,
    ) -> typing.Tuple[int, typing.List[typing.Dict[str, str]]]:
        fake_ctx = FakeContext(self.bot)
        try:
            await action(fake_ctx, *args, **kwargs)
        except Exception as e:
            notifications = fake_ctx.get_notifications()
            has_success = any(n["category"] == "success" for n in notifications)
            notifications.append(
                {
                    "message": _(
                        "An internal exception was raised after the operation: {error}"
                    ).format(error=str(e)),
                    "category": "warning" if has_success else "danger",
                }
            )
            return (0 if has_success else 1), notifications
        notifications = fake_ctx.get_notifications()
    ...
    async def add_repo(
        ...
        if (Downloader := self.bot.get_cog("Downloader")) is None:
            return {"status": 1}
        status, notifications = await self._safe_downloader_call(
            Downloader._repo_add, name=name, repo_url=repo_url, branch=branch
        )
        return {
            "status": status,
            "notifications": notifications,
    ...
    async def update_repos(self, user_id: int) -> typing.Dict[str, typing.Union[int, str]]:
        ...
        if (Downloader := self.bot.get_cog("Downloader")) is None:
            return {"status": 1}
        status, notifications = await self._safe_downloader_call(Downloader._repo_update)
        return {
            "status": status,
            "notifications": notifications,
        }
    ...
    async def update_cogs(self, user_id: int) -> typing.Dict[str, typing.Union[int, str]]:
        ...
        if (Downloader := self.bot.get_cog("Downloader")) is None:
            return {"status": 1}
        status, notifications = await self._safe_downloader_call(Downloader._cog_update_logic)
        return {
            "status": status,
            "notifications": notifications,
        }
    ...
    async def update_cogs_from_repo(
        ...
        if (repo := Downloader._repo_manager.get_repo(name)) is None:
            ...
            status, notifications = await self._safe_downloader_call(
                Downloader._cog_update_logic, repo=repo
            )
        return {
            "status": status,
            "notifications": notifications,
    ...
    async def install_cog(
        ...
        if (repo := Downloader._repo_manager.get_repo(repo_name)) is None:
            ...
            status, notifications = await self._safe_downloader_call(
                Downloader._cog_installrev, repo=repo, rev=None, cog_names=[name]
            )
        return {
            "status": status,
            "notifications": notifications,
    ...
    async def update_cog(
        ...
        if (repo := Downloader._repo_manager.get_repo(repo_name)) is None:
            ...
            status, notifications = await self._safe_downloader_call(
                Downloader._cog_update_logic, cogs=[cog]
            )
        return {
            "status": status,
            "notifications": notifications,
    ...
    async def pin_or_unpin_cog(
        ...
        if (repo := Downloader._repo_manager.get_repo(repo_name)) is None:
            ...
            status, notifications = await self._safe_downloader_call(
                Downloader._cog_pin if not cog.pinned else Downloader._cog_unpin, cog
            )
        return {
            "status": status,
            "notifications": notifications,
