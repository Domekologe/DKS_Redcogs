...
│                              │         │                                 │
│  ┌────────────────────────┐  │  JSON-  │  ┌───────────────────────────┐  │
│  │ webdashboard (Cog)     │  │  RPC    │  │ SvelteKit Server (BFF)    │  │
...
- **Companion-Cog `webdashboard`** (in diesem Repo, `DKS_Redcogs`): läuft im Bot-Prozess
  ...
  **Integrations-Registry**, in der sich andere Cogs mit Widgets, Panels und Seiten
  registrieren.
- **Web-App `DKS_Redbot_WebApp`** (separates Projekt, `W:\_Git\DKS_Redbot_WebApp`):
...
| Aspekt | AAA3A Red-Web-Dashboard | DKS Web Dashboard |
|---|---|---|
...
### 3.4 Datenfluss
1. Beim Cog-Load registriert sich der Dritt-Cog bei `webdashboard` über
...
```
DKS_Redcogs/webdashboard/        # Companion-Cog (dieses Repo)
  __init__.py
  info.json
  webdashboard.py                # Hauptcog (Lifecycle, Config, Commands)
...
DKS_Redbot_WebApp/               # Web-App (separates Projekt)
  src/
